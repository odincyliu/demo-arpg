...
static func _core_components() -> Array[SkillComponent]:
    return [
        ...
        _core(&"core_frost_lance", "Frost Lance", "A fast cold spell projectile.",
            ...
            [_op(&"damage", &"SET", 27.0), _op(&"cooldown", &"SET", 0.48), _op(&"projectile_speed", &"SET", 18.0), _op(&"target_range", &"SET", 12.0)]),
        _core(&"core_flame_orb", "Flame Orb", "A black flame projectile that bursts on impact or expiry.",
            ...
            [_op(&"damage", &"SET", 34.0), _op(&"cooldown", &"SET", 0.55), _op(&"projectile_speed", &"SET", 13.0), _op(&"impact_radius", &"SET", 1.8), _op(&"target_range", &"SET", 11.0)]),
        _core(&"core_frost_nova", "Frost Nova", "A cold spell that expands from the caster.",
            ...
            [_op(&"damage", &"SET", 26.0), _op(&"cooldown", &"SET", 0.62), _op(&"area_radius", &"SET", 4.0), _op(&"target_range", &"SET", 4.0)]),
        ...
        _core(&"core_void_rift", "Void Rift", "A persistent chaos damage area.",
            ...
            [_op(&"damage", &"SET", 9.0), _op(&"cooldown", &"SET", 0.9), _op(&"area_radius", &"SET", 3.0), _op(&"persistent_duration", &"SET", 3.0), _op(&"persistent_tick_interval", &"SET", 0.35), _op(&"target_range", &"SET", 10.0)], &"target"),
        _core(&"core_summon", "Summon", "Create a temporary ranged follower.",
            ...
            [_op(&"damage", &"SET", 14.0), _op(&"cooldown", &"SET", 1.5), _op(&"summon_duration", &"SET", 10.0), _op(&"summon_attack_interval", &"SET", 0.8), _op(&"target_range", &"SET", 8.0)], &"event"),
    ]
